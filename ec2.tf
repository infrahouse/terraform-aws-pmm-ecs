...
resource "aws_cloudwatch_metric_alarm" "pmm_auto_recovery" {
  ...
  alarm_description   = "Auto recover PMM instance when system status check fails"
  namespace           = "AWS/EC2"
...
resource "aws_cloudwatch_metric_alarm" "pmm_instance_status" {
  alarm_name          = "${local.service_name}-instance-status"
  alarm_description   = "Alert when PMM instance status check fails"
  namespace           = "AWS/EC2"
...
resource "aws_cloudwatch_metric_alarm" "pmm_high_cpu" {
  alarm_name          = "${local.service_name}-high-cpu"
  alarm_description   = "Alert when PMM instance CPU usage is high"
  namespace           = "AWS/EC2"
